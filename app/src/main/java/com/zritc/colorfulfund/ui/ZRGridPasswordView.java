...
public class ZRGridPasswordView extends LinearLayout implements ZRPasswordView {
    ...
    private ZRImeDelBugFixedEditText mInputView;
    private OnPasswordChangedListener mListener;
    private PasswordTransformationMethod mTransformationMethod;

    public ZRGridPasswordView(Context context) {
        super(context);
        initViews(context);
        init(context, null, 0);
    }

    public ZRGridPasswordView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public ZRGridPasswordView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ZRGridPasswordView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ZRGridPasswordView, defStyleAttr, 0);

        mTextColor = ta.getColorStateList(R.styleable.ZRGridPasswordView_gpvTextColor);
        if (mTextColor == null)
            mTextColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.ZRGridPasswordView_gpvTextSize, -1);
        if (textSize != -1) {
            this.mTextSize = ZRDeviceInfo.px2sp(context, textSize);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.ZRGridPasswordView_gpvLineWidth, ZRDeviceInfo.dp2px(1));
        mLineColor = ta.getColor(R.styleable.ZRGridPasswordView_gpvLineColor, DEFAULT_LINECOLOR);
        mGridColor = ta.getColor(R.styleable.ZRGridPasswordView_gpvGridColor, DEFAULT_GRIDCOLOR);
        mLineDrawable = ta.getDrawable(R.styleable.ZRGridPasswordView_gpvLineColor);
        if (mLineDrawable == null)
            mLineDrawable = new ColorDrawable(mLineColor);
        mOuterLineDrawable = generateBackgroundDrawable();

        mPasswordLength = ta.getInt(R.styleable.ZRGridPasswordView_gpvPasswordLength, DEFAULT_PASSWORDLENGTH);
        mPasswordTransformation = ta.getString(R.styleable.ZRGridPasswordView_gpvPasswordTransformation);
        if (TextUtils.isEmpty(mPasswordTransformation))
            mPasswordTransformation = DEFAULT_TRANSFORMATION;


        mPasswordType = ta.getInt(R.styleable.ZRGridPasswordView_gpvPasswordType, 0);
    ...
    private void initViews(Context context) {
        ...
        mTransformationMethod = new ZRCustomPasswordTransformationMethod(mPasswordTransformation);
    ...
    private void inflaterViews(Context context) {
        ...
        mInputView = (ZRImeDelBugFixedEditText) findViewById(R.id.inputView);
    ...
    private ZRImeDelBugFixedEditText.OnDelKeyEventListener onDelKeyEventListener = new ZRImeDelBugFixedEditText.OnDelKeyEventListener() {
