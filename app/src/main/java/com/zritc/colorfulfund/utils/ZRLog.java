...
public class ZRLog {
	...
	private static final String LOG_FILE_PREFIX = "mdsjr_1_0_log";
	private static final String CRASH_FILE_PREFIX = "mdsjr_1_0_crash";
	...
	public static void writeLogFile(String tag, String msg, Throwable ex) {
		try {
			...
			msg = ZRUtils.getCurrentTime(ZRUtils.TIME_FORMAT2) + " --- " + tag
					+ " --- " + msg + "\n";
	...
	public static void writeCrashFile(Throwable ex) {
		try {
			...
			String msg = ZRUtils.getCurrentTime(ZRUtils.TIME_FORMAT2) + "\r\n";
	...
	private static synchronized boolean checkCurrentFile(int type)
		...
		String currdate = ZRUtils.getCurrentTime(ZRUtils.TIME_FORMAT4);
		...
		if (null == current || LOG_FILE_SIZE <= current.length()) {
			 ...
			 */
			if (null == current) {
				String currentTime = ZRUtils
						.getCurrentTime(ZRUtils.TIME_FORMAT);
				if (FILE_TYPE_LOG == type) {
